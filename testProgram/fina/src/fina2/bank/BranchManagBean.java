/*
 * BranchManagBean.java
 *
 * Created on April 23, 2002, 8:00 PM
 */
 ...
 * @version
 */
public class BranchManagBean implements EntityBean {

	private EntityContext ctx;
	public BranchManagPK pk;
	...
	public BranchManagPK ejbCreate() throws EJBException, CreateException {
		BranchManagPK pk = null;
		...
		try {
			...
			ps = con.prepareStatement("select max(id) from IN_BRANCH_MANAGEMENT");

			insert = con.prepareStatement("insert into IN_BRANCH_MANAGEMENT ( " + "id, nameStrID, lastNameStrID, postStrID, registrationStrId1, "
					+ "registrationStrId2, registrationStrId3, commentsStrID1, commentsStrID2) " + "values(?,?,?,?,?,?,?,?,?)");
			rs = ps.executeQuery();

			if (rs.next())
				pk = new BranchManagPK(rs.getInt(1) + 1);
			else
				pk = new BranchManagPK(1);
	...
	public BranchManagPK ejbFindByPrimaryKey(BranchManagPK pk) throws EJBException, FinderException {
		Connection con = DatabaseUtil.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			ps = con.prepareStatement("select id from IN_BRANCH_MANAGEMENT where id=?");
			ps.setInt(1, pk.getId());
			rs = ps.executeQuery();

			if (!rs.next()) {
				throw new FinderException("Branch Manager is not found.");
			}
	...
	public void ejbStore() throws javax.ejb.EJBException, java.rmi.RemoteException {
		...
		PreparedStatement ps = null;
		try {
			ps = con.prepareStatement("update IN_BRANCH_MANAGEMENT set " + "nameStrID=?,  " + "lastNameStrID=?, " + "managingBodyID=?, " + "postStrID=?, " + "phone=?, " + "dateOfAppointment=?, "
					+ "cancelDate=?, " + "registrationStrId1=?, " + "registrationStrId2=?, " + "registrationStrId3=?, " + "commentsStrId1=?, " + "commentsStrId2=?, " + "branchID=? " + "where id=?");
			ps.setInt(1, nameID);
			...
			ps.setInt(13, branchID);
			ps.setInt(14, ((BranchManagPK) ctx.getPrimaryKey()).getId());

			ps.executeUpdate();

			this.pk = (BranchManagPK) ctx.getPrimaryKey();
	...
	public void ejbLoad() throws javax.ejb.EJBException, java.rmi.RemoteException {
		...
		try {
			ps = con.prepareStatement("select nameStrID, lastNameStrID, managingBodyID, postStrID, phone, dateOfAppointment, cancelDate, "
					+ "registrationStrId1, registrationStrId2, registrationStrId3, commentsStrId1, commentsStrId2 from IN_BRANCH_MANAGEMENT where id=?");
			ps.setInt(1, ((BranchManagPK) ctx.getPrimaryKey()).getId());
	...
	public void ejbRemove() throws javax.ejb.RemoveException, javax.ejb.EJBException, java.rmi.RemoteException {
		...
		try {
			...
			ps = con.prepareStatement("select nameStrID,lastNameStrId,postStrId,registrationStrId1,registrationStrId2, "
					+ "registrationStrId3,commentsStrId1,commentsStrId2 from IN_BRANCH_MANAGEMENT where id=?");
			...
			ps = con.prepareStatement("delete from IN_BRANCH_MANAGEMENT " + "where id=?");
			ps.setInt(1, ((BranchManagPK) ctx.getPrimaryKey()).getId());
			ps.executeUpdate();
		} catch (Exception e) {
			throw new EJBException(e);
	...
	public BranchPK getBranchPK() throws RemoteException, EJBException {
		return new BranchPK(branchID);
	}

	public void setBranchPK(BranchPK branchPK) throws RemoteException, EJBException {
		branchID = branchPK.getId();
	}
}
