...
public class BankTypesFrame extends BaseFrame {
	...
	private BankTypesAmendAction amendAction;
	private BankTypesInsertAction insertAction;
	private BankTypesReviewAction reviewAction;
	private BankTypesDeleteAction deleteAction;
	...
	public BankTypesFrame() {
		...
		amendAction = new BankTypesAmendAction(main.getMainFrame(), table);
		insertAction = new BankTypesInsertAction(main.getMainFrame(), table);
		reviewAction = new BankTypesReviewAction(main.getMainFrame(), table);
		deleteAction = new BankTypesDeleteAction(main.getMainFrame(), table);
	...
	private void initTable() {
		try {
			...
			table.initTable(colNames, session.getBankTypesRows(main.getUserHandle(), main.getLanguageHandle()));

		} catch (Exception e) {
			Main.generalErrorHandler(e);
			ui.putConfigValue("fina2.bank.BankTypesFrame.visible", new Boolean(false));
		}
	}
	...
	private void initComponents() {// GEN-BEGIN:initComponents
		...
		if (ui.getHelpManager().IsHelpSystem()) {
			ui.getHelpManager().createDisplayHelpFromFocus(helpButton, "Fl_Types");
		} else {
	...
	protected void refreshButtonActionPerformed(java.awt.event.ActionEvent evt) { // event_refreshButtonActionPerformed
		initTable();
	}

	protected void closeButtonActionPerformed(java.awt.event.ActionEvent evt) { // event_closeButtonActionPerformed
...
class BankTypesAmendAction extends AbstractAction {

	private fina2.ui.UIManager ui = fina2.Main.main.ui;

	private BankTypeAmendDialog dialog;
	private java.awt.Frame parent;
	private EJBTable table;

	BankTypesAmendAction(java.awt.Frame parent, EJBTable table) {
		super();

		dialog = new BankTypeAmendDialog(parent, true);
...
class BankTypesInsertAction extends AbstractAction {
	...
	BankTypesInsertAction(java.awt.Frame parent, EJBTable table) {
		super();
...
class BankTypesReviewAction extends AbstractAction {
	...
	BankTypesReviewAction(java.awt.Frame parent, EJBTable table) {
		super();
...
class BankTypesDeleteAction extends AbstractAction {
	...
	BankTypesDeleteAction(java.awt.Frame parent, EJBTable table) {
		super();
	...
	public void actionPerformed(java.awt.event.ActionEvent actionEvent) {
		if (!ui.showConfirmBox(parent, ui.getString("fina2.bank.bankTypeDeleteQuestion")))
			return;
		...
		try {
			...
			bankType = home.findByPrimaryKey((BankTypePK) table.getSelectedPK());
			bankType.remove();

			table.removeRow(index);
		} catch (Exception e) {
			ui.showMessageBox(parent, ui.getString("fina2.delete.bankType"));
			Main.errorHandler(parent, Main.getString("fina2.title"), Main.getString("fina2.delete.bankType"));
		}
	}
}
